...
describe('TV Parity API and UI contracts', () => {
  ...
  it('keeps TV parity inputs app-controlled instead of browser-locale editable controls', () => {
    ...
    expect(enMessages.tvParity.strategyContext).toBe('Strategy market context')
    expect(enMessages.tvParity.dropFiles).toBeTruthy()
    expect(enMessages.tvParity.dropFilesHint).toBeTruthy()
    expect(enMessages.tvParity.fullPrehistory).toBe('Full pre-history')
    expect(pageSource).toContain('selectedStrategyMarketContext')
    expect(pageSource).toContain("t('tvParity.strategyContext')")
    expect(pageSource).toContain("t('tvParity.dropFiles')")
    expect(pageSource).toContain("t('tvParity.dropFilesHint')")
    expect(pageSource).toContain('ref="unifiedFileInput"')
    expect(pageSource).toContain('multiple')
    expect(pageSource).toContain('@drop.prevent="handleDrop"')
    expect(pageSource).toContain('@change="onUnifiedInputChange"')
    ...
    expect(pageSource).toContain("t('tvParity.fullPrehistory')")
  })
})
