...
describe('global UI correctness contracts', () => {
  ...
  it('exposes first-class job and object routes plus 404', () => {
    const router = source('router/index.ts')
    const en = JSON.parse(source('i18n/locales/en.json'))
    const ru = JSON.parse(source('i18n/locales/ru.json'))
    expect(router).toContain("/jobs/:jobId")
    expect(router).toContain("/strategies/:strategyId")
    expect(router).toContain("/backtests/:runId")
    expect(router).toContain("/tv-parity/:runId")
    expect(router).toContain("'not-found'")
    expect(en.app.notFoundTitle).toBeTruthy()
    expect(ru.app.notFoundTitle).toBeTruthy()
