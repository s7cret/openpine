  ...
  <section class="space-y-4">
    <div>
      ...
      <p class="mt-1 text-sm text-gray-400">
        {{ te('optimizer.placeholder') ? t('optimizer.placeholder') : 'Validate a parameter search before launching trials.' }}
      </p>
      <p v-if="jobId" class="mt-1 text-xs text-gray-500">{{ jobId }}</p>
    </div>
    <div class="grid gap-3 rounded-xl border border-dark-500 bg-dark-800 p-4 md:grid-cols-3">
      ...
      <label class="text-sm text-gray-300">
        {{ te('optimizer.trials') ? t('optimizer.trials') : 'Trials' }}
      ...
      <div class="flex items-end">
        <button class="rounded-lg bg-accent px-4 py-2 text-sm text-white disabled:opacity-50" :disabled="loading" @click="validate">
          {{ te('optimizer.validate') ? t('optimizer.validate') : 'Validate config' }}
        </button>
      </div>
    </div>
