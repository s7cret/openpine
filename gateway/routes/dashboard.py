...
async def dashboard(
    ...
    jobs = JobSummary(
        pending=sum(1 for j in all_jobs if j.status == JobStatus.PENDING) + _count_jobs(persistent_jobs, "pending"),
        running=sum(1 for j in all_jobs if j.status == JobStatus.RUNNING) + _count_jobs(persistent_jobs, "running"),
        done=sum(1 for j in all_jobs if j.status == JobStatus.DONE) + _count_jobs(persistent_jobs, "done"),
        failed=sum(1 for j in all_jobs if j.status == JobStatus.FAILED) + _count_jobs(persistent_jobs, "failed"),
        recent=sorted(recent_jobs, key=lambda x: x.get("created_at") or 0, reverse=True)[:20],
    )
