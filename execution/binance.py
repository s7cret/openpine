...
class BinanceLiveExecutionAdapter:
    ...
    def _parse_client_response(
        ...
        err_msg = None if is_success else f"Unknown exchange status '{response.get('status', 'unknown')}'"
    ...
    async def reconcile(self, account_id: str) -> list[Order]:
        ...
        """
        if self._client is None:
            raise ExecutionUnavailableError("Binance reconcile requires an injected client")
