...
class StdlibHTTPTransport:
    ...
    def _post_json(self, token: str, method: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def get_file(self, token: str, file_id: str) -> dict[str, Any]:
        ...
        """
        return self._post_json(token=token, method="getFile", payload={"file_id": file_id})
...
class TelegramPluginConfig:
    ...
    def resolve_token(self) -> str | None:
        ...
        if not self.token_ref.startswith("env:"):
            raise ValueError(f"token_ref must be 'env:VAR' format, got: {self.token_ref!r}")
...
class TelegramCommandPlugin:
    ...
    def get_updates(
        ...
        return [TelegramUpdate.from_api(item) for item in result if isinstance(item, dict)]
...
class TelegramBotHandler:
    ...
    async def run(self, poll_interval: float = 1.0) -> None:
        ...
        logger.info("telegram_bot_starting", cli_path=self.cli_path, poll_interval=poll_interval)
    ...
    def _poll_once(self) -> int:
        ...
        for update in updates:
            try:
                ...
                logger.error("telegram_process_update_error",
                             update_id=update.update_id, error=str(exc))
    ...
    def _process_update(self, update: TelegramUpdate) -> None:
        ...
        if update.message is not None and update.text is not None and update.text.startswith("/"):
    ...
    def _handle_callback_query(self, update: TelegramUpdate) -> None:
        ...
        try:
            ...
            logger.warning("telegram_callback_error",
                           callback_data=callback_data, error=str(exc))
    ...
    def _render_menu_callback(self, callback_data: str, chat_id: str, message_id: int | None) -> None:
    ...
    def _run_cli_and_respond(self, argv: list[str], chat_id: str, callback_data: str) -> None:
    ...
    def _handle_document_message(self, update: TelegramUpdate) -> None:
        ...
        try:
            ...
            if not file_info.get("ok"):
                self._send_message(chat_id, f"Failed to get file: {file_info.get('description')}")
                return

            result_obj = file_info.get("result", {})
            file_path = result_obj.get("file_path") if isinstance(result_obj, dict) else None
            if not file_path:
                self._send_message(chat_id, "Failed to retrieve file path from Telegram.")
                return
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                if not result.get("ok"):
                    logger.warning("telegram_edit_reply_markup_error",
