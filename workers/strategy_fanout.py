...
class StrategyBarFanout:
    ...
    def process_source_bar(self, bar: Bar) -> SourceBarFanoutResult:
        ...
        strategies = _enabled_strategies_for_market(self.registry.list_strategies(), market_key)
        ...
        if not strategies:
            return SourceBarFanoutResult(
                ...
                strategies=0,
                targets=(TargetBarResult(source_timeframe.canonical, FanoutStatus.NO_STRATEGIES),),
            )
        ...
        for group in _group_by_target_timeframe(strategies):
            ...
            if target_bar is None:
                targets.append(TargetBarResult(group.timeframe, FanoutStatus.TARGET_NOT_CLOSED))
                continue
            ...
            jobs = tuple(self._enqueue_strategy_job(strategy, target_bar, market_key) for strategy in group.strategies)
            targets.append(TargetBarResult(group.timeframe, FanoutStatus.ENQUEUED, bar=target_bar, jobs=jobs))
    ...
    def _target_bar_from_source(self, source_bar: Bar, target_timeframe: str) -> Bar | None:
        source_timeframe = parse_timeframe(self.config.source_timeframe)
        target = parse_timeframe(target_timeframe)
        if target.duration_ms is None or source_timeframe.duration_ms is None:
            raise ValueError(f"variable duration timeframe is not supported for fanout: {target_timeframe}")
        if target.duration_ms < source_timeframe.duration_ms:
            raise ValueError(f"target timeframe {target.canonical} is below source {source_timeframe.canonical}")
        if target.duration_ms % source_timeframe.duration_ms != 0:
            raise ValueError(f"target timeframe {target.canonical} is not a multiple of source {source_timeframe.canonical}")
...
def _group_by_target_timeframe(strategies: Iterable[StrategyInstance]) -> tuple[_StrategyGroup, ...]:
