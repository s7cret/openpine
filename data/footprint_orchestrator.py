...
class FootprintOrchestrator:
    ...
    def load_footprints(self, query: FootprintQuery) -> FootprintSeries:
        if self._provider is not None and query.source in {"auto", "provider"}:
            ...
            if query.gap_policy == "fail" and not series.coverage.is_complete:
                raise IncompleteCoverageError(f"footprint coverage incomplete: {series.coverage.missing_intervals}")
        ...
        if query.gap_policy == "fail" and not series.coverage.is_complete:
            raise IncompleteCoverageError(f"footprint storage coverage incomplete: {series.coverage.missing_intervals}")
        return series
    ...
    def store_footprints(self, series: FootprintSeries) -> StoreResult:
        ...
        if not result.success:
            raise StorageUnavailableError(result.error or "failed to persist footprints")
        return result
