...
class DirectBinanceProvider:
    ...
    def effective_query(self, query: BarQuery) -> tuple[BarQuery, int | None]:
        ...
        effective_start = max(query.start_ms, earliest) if earliest is not None else query.start_ms
        return self._copy_query(query, start_ms=effective_start), earliest

    @staticmethod
    def estimate_bars(query: BarQuery, *, start_ms: int | None = None, end_ms: int | None = None) -> int:
    ...
    @classmethod
    def estimate_pages(cls, query: BarQuery, *, start_ms: int | None = None, end_ms: int | None = None) -> int:
        return math.ceil(cls.estimate_bars(query, start_ms=start_ms, end_ms=end_ms) / BINANCE_PAGE_LIMIT)
    ...
    def fetch_bars(self, query: BarQuery, progress_callback=None) -> BarSeries:
        ...
        if progress_callback:
            progress_callback(0, 0, total_bars, total_pages, earliest_open_ms, "cache_lookup")
        ...
        for page_num in range(max_pages):
            ...
            if progress_callback and (page_num + 1) % 10 == 0:
                progress_callback(len(all_bars), page_num + 1, total_bars, total_pages, earliest_open_ms, "fetch")
        ...
        if progress_callback:
            fetched_pages = min(math.ceil(len(all_bars) / BINANCE_PAGE_LIMIT), total_pages)
            progress_callback(len(all_bars), fetched_pages, total_bars, total_pages, earliest_open_ms, "fetch_done")
