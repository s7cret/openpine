...
class BybitLiveExecutionAdapter(CcxtOrderClientMixin):
    ...
    def _parse_client_response(
        ...
        order_id = (
            response.get("id") or response.get("orderId") or response.get("order_id")
        )
    ...
    def _result_to_order(
        ...
        if result.success:
            ...
            return rejected_live_order(
                ...
                order_id=order_id,
                now=now,
                error=result.error or "Bybit adapter returned unsuccessful order",
            )
    ...
    async def get_order_status_for_symbol(
        ...
        if self._client is None:
            raise ExecutionUnavailableError(
                "Bybit order status requires an injected client"
            )
        ...
        try:
            ...
            raise ExecutionUnavailableError(
                f"Bybit order status failed: {exc}"
            ) from exc
    ...
    async def reconcile(self, account_id: str) -> list[Order]:
        ...
        if self._client is None:
            raise ExecutionUnavailableError(
                "Bybit reconcile requires an injected client"
            )
