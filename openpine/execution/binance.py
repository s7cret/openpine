"""Binance live execution adapter for OpenPine — Phase 7.

Fail-closed live exchange adapter for Binance.
...
# Binance ccxt status mapping — Binance uses specific status strings
_BINANCE_STATUS_MAP: dict[str, str] = {
    "new": "new",
    "filled": "filled",
    "partial": "partial",
    "canceled": "cancelled",
...
def _map_binance_status(raw: str) -> str:
    """Map Binance exchange status to normalized status string."""
    return _BINANCE_STATUS_MAP.get(raw.lower(), raw.lower())


class BinanceLiveExecutionAdapter(CcxtOrderClientMixin):
    """Binance live execution adapter — fail-closed by default.
    ...
    def __init__(
        ...
        Args:
            client: Async Binance client (e.g., ccxt-binance async).
    ...
    async def submit_order(self, order: OrderIntent) -> Order:
        """Submit order to Binance via injected client.
    ...
    def _parse_client_response(
        ...
        # Standard ccxt response fields
        order_id = response.get("id") or response.get("orderId")
        status = _map_binance_status(response.get("status", "unknown"))
        ...
        is_success = status in ("new", "filled", "partial")
        err_msg = (
            None
            if is_success
            else f"Unknown exchange status '{response.get('status', 'unknown')}'"
        )
    ...
    async def cancel_order_for_symbol(self, order_id: str, symbol: str) -> bool:
        ...
        if self._client is None:
            raise ExecutionUnavailableError(
                "Binance cancel requires an injected client"
            )
    ...
    async def _call_cancel_order(self, order_id: str, symbol: str) -> CancelResult:
        ...
        return CancelResult(
            success=_map_binance_status(response.get("status", "")) == "cancelled",
        )
    ...
    async def get_order_status_for_symbol(
        ...
        try:
            ...
            raise ExecutionUnavailableError(
                f"Binance order status failed: {exc}"
            ) from exc
    ...
    def _response_to_order_status(self, response: Any) -> Order:
        ...
        mapped_status = _map_binance_status(raw_status)
    ...
    async def reconcile(self, account_id: str) -> list[Order]:
        ...
        try:
            ...
            raise ExecutionUnavailableError(f"Binance reconcile failed: {exc}") from exc
