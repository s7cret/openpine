...
def compile_source(source: Any, *, force: bool) -> tuple[str | None, dict[str, Any]]:
    ...
    if source.active_artifact_id and not force:
        return source.active_artifact_id, {
            "status": "cached",
...
def timed_call(
    timings: dict[str, float], key: str, fn: Any, *args: Any, **kwargs: Any
) -> Any:
...
def ensure_strategy_instance(
    entry: ExportEntry, source: Any, artifact_id: str, timeframe: str
) -> tuple[str, bool]:
...
def load_calculation_bars(
    ...
    calculation_to = (
        parse_time_ms(args.calculation_to)
        or calculation_to_by_tf.get(chart.timeframe)
        or chart_end_exclusive_ms(chart)
    )
    if calculation_from >= calculation_to:
        raise ValueError(
            f"invalid calculation window: {calculation_from} >= {calculation_to}"
        )
    cache_key = (
        args.symbol.upper(),
        args.exchange.upper(),
...
def _infer_tv_bar_index_offset_from_periodic_na(
    ...
    for column in df.columns:
        ...
        values = df[column]
        na_positions = [
            idx for idx, value in enumerate(values) if pd.isna(value) or value == ""
        ]
...
def _run_meta_valid(path: Path) -> bool:
    ...
    if not isinstance(revisions, dict):
        return False
    return all(
        isinstance(revisions.get(name), str)
        and revisions.get(name)
        and revisions.get(name) != "unknown"
        and not revisions.get(name, "").startswith("error:")
        for name in LIBRARY_NAMES
    )
...
def _run_summary_valid(path: Path) -> bool:
    ...
    try:
        ...
        return False
    return (
        summary.get("schema_version") == RUN_META_SCHEMA_VERSION
...
def _run_windows(
    chart: ChartExport, run_info: dict[str, Any]
...
def _register_entry_strategies(
    ...
    for chart in entry.charts:
        if args.timeframe and chart.timeframe != normalize_tf(args.timeframe):
            continue
        sid, created = ensure_strategy_instance(
            entry, source, artifact_id, chart.timeframe
        )
        registered.append(
            {"timeframe": chart.timeframe, "strategy_id": sid, "created": created}
        )
    return registered
...
def _run_entry_charts(
    ...
    for chart in entry.charts:
        ...
        try:
            if entry.kind == "indicator":
                run_info = run_indicator(
                    entry, source, artifact_id, chart, out_dir, args
                )
            elif entry.kind == "strategy":
                run_info = run_strategy(
                    entry, source, artifact_id, chart, out_dir, args
                )
            else:
                run_info = {
                    "status": "skipped",
                    "reason": f"unsupported kind {entry.kind}",
                }
        except Exception as exc:
            run_info = {
                "status": "run_error",
        ...
        write_json(
            ...
            _build_run_summary(
                entry=entry, chart=chart, run_meta=run_meta, run_info=run_info
            ),
        )
...
def run_entry(
    ...
    args: argparse.Namespace,
    batch_id: str = "",
    library_revisions: dict[str, str] | None = None,
) -> dict[str, Any]:
    ...
    source, added = timed_call(
        timings, "ingest_sec", get_or_add_source, entry, write=True
    )
    ...
    artifact_id, compile_info = timed_call(
        timings, "compile_sec", compile_source, source, force=args.force_compile
    )
    status["compile"] = {
    ...
    status["runs"] = runs
    status["status"] = (
        "ok" if all(r.get("status") == "ok" for r in runs) else "partial_or_error"
    )
    return _finish_entry_status(status, started)
...
def resolve_calculation_to_by_timeframe(
    ...
    for entry in entries:
        for chart in entry.charts:
            if args.timeframe and chart.timeframe != normalize_tf(args.timeframe):
                continue
            out[chart.timeframe] = max(
                out.get(chart.timeframe, 0), chart_end_exclusive_ms(chart)
            )
    return out
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--kind", choices=["all", "indicator", "strategy"], default="all"
    )
    parser.add_argument(
        "--timeframe", default=None, help="Optional timeframe filter: 15m, 1D, 1h"
    )
    parser.add_argument("--limit", type=int, default=None)
    parser.add_argument("--start-id", type=int, default=None)
    parser.add_argument(
        "--ids", default=None, help="Comma/range list, e.g. 1,2,1501-1528"
    )
    ...
    parser.add_argument(
        "--skip-completed",
        action="store_true",
        help="Skip entries already completed for the selected phase.",
    )
    ...
    parser.add_argument(
        "--calculation-to",
        default=None,
        help="Optional calculation end. Default: TV visible window end.",
    )
    parser.add_argument(
        "--provider-only-bars",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10_000,
        help="Runtime bar progress interval. 0 disables.",
    )
    ...
    return {
        ...
        "calculation_to_by_timeframe": {
            ...
            for tf, value in sorted(
                getattr(args, "_calculation_to_by_timeframe", {}).items()
            )
        },
...
def _run_selected_entries(
    *,
    ...
    for idx, entry in enumerate(selected, 1):
        ...
        print(
            f"[{idx}/{len(selected)}] {entry.export_id:04d} {entry.kind} {entry.folder}"
        )
        ...
        try:
            result = run_entry(
                entry, args, batch_id=batch_id, library_revisions=library_revisions
            )
        except Exception as exc:
            result = {
                **entry_summary(entry),
        ...
        write_json(
            entry.root / "openpine_outputs" / "openpine_batch_status.json",
            status_record,
        )
        ...
        if args.stop_on_error and result.get("status") in {
            "compile_error",
            "fatal_error",
            "partial_or_error",
        }:
...
def main(argv: list[str] | None = None) -> int:
    ...
    final_status = (
        "completed"
        if all(r.get("status") != "fatal_error" for r in results)
        else "failed"
    )
