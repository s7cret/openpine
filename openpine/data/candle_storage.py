...
PARQUET_SCHEMA = parquet.schema(
    [
        ...
        ("timeframe", "string"),
        ("open_time", "int64"),
        ("close_time", "int64", True),
        ("open", "float64"),
        ("high", "float64"),
        ("low", "float64"),
        ("close", "float64"),
        ("volume", "float64", True),
        ("quote_volume", "float64", True),
        ("trades_count", "int64", True),
        ("is_closed", "bool"),
        ("source", "string"),
        ("provider", "string"),
        ("ingested_at", "int64"),
...
def _resolve_write_identity(
    ...
    if bar_instrument_key is None:
        raise ValueError(
            "instrument_key is required either as parameter or bar attribute"
        )
    try:
...
def _row_to_bar(row: dict, query: BarQuery) -> Bar:
    ...
    return Bar(
        ...
        time=open_time,
        time_close=(
            open_time + query.timeframe.duration_ms
            if _row_value_is_missing(close_time)
            else int(close_time)
        ),
        open=float(row["open"]),
        high=float(row["high"]),
...
class CandleStorage:
    ...
    def _validate_path_component(self, component: str) -> None:
        ...
        if (
            ...
            raise ValueError(f"Path component escapes candle storage root: {component}")

    def _path_under_candles_root(self, *parts: str) -> Path:
        for part in parts:
            self._validate_path_component(part)
        path = self.candles_root.joinpath(*parts)
        root = self.candles_root.resolve()
        resolved = path.resolve(strict=False)
        try:
            resolved.relative_to(root)
        except ValueError as exc:
            raise ValueError(f"Path escapes candle storage root: {path}") from exc
        return path
    ...
    def _partition_path(
        ...
        return self._path_under_candles_root(
            f"exchange={exchange}",
    ...
    def write_candles(
        ...
        try:
            exchange, market_type, symbol, price_type = _resolve_write_identity(
    ...
    def _write_partition(
        ...
        *,
        ...
        parquet.write_dataframe(df, tmp_path, schema=PARQUET_SCHEMA, compression="zstd")
    ...
    def read_candles(self, query: BarQuery) -> list["Bar"]:
        ...
        """
        _storage_identity(query)

        # Find matching manifest files
        manifests = self.list_manifests(query)
        if not manifests:
            raise _storage_unavailable(
                f"no candle manifests for query: {_storage_instrument_key(query)}"
            )
        ...
        for m in manifests:
            ...
            df = parquet.read_dataframe(pq_path)

            # Apply time filter
            df = df[
                (df["open_time"] >= query.start_ms) & (df["open_time"] < query.end_ms)
            ]
        ...
        if not all_rows:
            raise _storage_unavailable(
                f"no candle rows for query: {_storage_instrument_key(query)}"
            )

        return [
            _row_to_bar(row, query) for row in _deduplicate_rows_by_open_time(all_rows)
        ]
    ...
    def list_manifests(self, query: BarQuery) -> list[CandleManifest]:
        ...
        exchange, market_type, symbol, price_type, timeframe, start_ms, end_ms = (
            _storage_identity(query)
        )
    ...
    def detect_gaps(self, query: BarQuery) -> list[DataGap]:
        ...
        gaps: list[DataGap] = []
        exchange, market_type, symbol, price_type, timeframe, _, _ = _storage_identity(
            query
        )
    ...
    def close(self) -> None:
        """Close the lazy SQLite manifest connection, if opened."""
