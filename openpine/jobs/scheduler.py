...
from openpine.jobs.models import Job, JobStatus
...
class JobScheduler:
    ...
    _running: dict[str, str] = field(
        default_factory=dict
    )  # serialization_key -> job_id
    _locks: dict[str, tuple[str, int]] = field(
        default_factory=dict
    )  # resource -> (owner, expiry_ms)
    _idempotency_map: dict[str, str] = field(
        default_factory=dict
    ...
    def mark_done(self, job_id: str, result: dict | None = None) -> None:
        ...
        with self._lock:
            ...
            if (
                ...
                and self._running.get(job.serialization_key) == job_id
            ):
