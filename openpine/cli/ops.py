...
def jobs_list() -> None:
    ...
    for j in jobs:
        console.print(
            f"  [{j.status.value}] {j.id[:8]}  type={j.job_type.value}  "
            f"strategy={getattr(j, 'strategy_id', '-')}  "
            f"created={_fmt_utc_ms_as(j.created_at, '%H:%M:%S')}"
        )
...
def jobs_retry(job_id: str) -> None:
    ...
    if job.status != JobStatus.FAILED:
        console.print(
            f"[yellow]Job {job_id} is not failed (status={job.status}). Cannot retry.[/yellow]"
        )
        sys.exit(1)
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would be enqueued without creating"
)
def jobs_enqueue_live_bar(
    ...
    job = Job(
        ...
        idempotency_key=f"live-bar:{strategy}:{bar_time}",
        serialization_key=strategy,
        status=JobStatus.PENDING,
    )

    if dry_run:
        console.print("[dim]Would enqueue job:[/dim]")
        console.print(f"  type:        {job.job_type.value}")
        console.print(f"  strategy:    {job.strategy_id}")
        console.print(f"  bar_time:    {bar_time}")
        console.print(
            "  → would be enqueued (no existing job with this idempotency_key)"
        )
        return
...
[Service]
Type=simple
WorkingDirectory=%h/.openpine
Environment=OPENPINE_HOME=%h/.openpine
EnvironmentFile=-%h/.config/openpine/openpine.env
...
"""
    ...
    console.print("  1. Reload systemd:    systemctl --user daemon-reload")
    console.print("  2. Enable service:    openpine service enable")
    console.print("  3. Start service:    openpine service start")
