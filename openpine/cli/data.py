...
from openpine.timezones import parse_ymd_ms
...
def _parse_cli_ymd_ms(value: str, *, option_name: str) -> tuple[int | None, str | None]:
    try:
        return parse_ymd_ms(value), None
    except ValueError:
...
def _parse_data_backfill_window(
    *,
    ...
    if start_ms is None:
        raise RuntimeError("--from date parser returned no timestamp")
    if to_date:
        end_ms, error = _parse_cli_ymd_ms(to_date, option_name="--to")
        if error:
            return None, None, error
        if end_ms is None:
            raise RuntimeError("--to date parser returned no timestamp")
    else:
        end_ms = now_ms
...
def _run_sync_marketdata_backfill(
    *,
    ...
    try:
        previous_handler = (
            signal.signal(signal.SIGALRM, _raise_timeout) if timeout > 0 else None
        )
        if timeout > 0:
    ...
    console.print(
        f"[green]Backfill complete: {len(series.bars)} candles available[/green]"
    )
    return True
    ...
    try:
        ...
        if rows:
            ...
            for row in rows:
                ...
                updated_str = (
                    _fmt_utc_ms_as(updated_at, "%Y-%m-%d %H:%M")
                    if updated_at
                    else "N/A"
                )
                tbl.add_row(
                    exch, sym, timeframe, provider or "-", status or "-", updated_str
                )
            console.print(tbl)
        else:
...
def data_gaps(symbol: str, timeframe: str, exchange: str, market: str) -> None:
    ...
    console.print(
        f"[bold]Data gaps[/bold] {symbol} {timeframe} (exchange={exchange}, market={market})"
    )
...
@click.argument("timeframe", required=True)
@click.option(
    "--from", "from_ts", required=True, type=int, help="Gap start timestamp (ms)"
)
@click.option("--to", "to_ts", required=True, type=int, help="Gap end timestamp (ms)")
@click.option("--exchange", default="binance", help="Exchange name")
def data_repair(
    symbol: str, timeframe: str, from_ts: int, to_ts: int, exchange: str
) -> None:
    ...
    if invalidated:
        console.print(
            f"[yellow]Affected strategies paused/marked for rebuild:[/yellow] {len(invalidated)}"
        )
...
@click.option(
    "--to", "to_date", default=None, help="End date (YYYY-MM-DD, default: today)"
)
@click.option("--exchange", default="binance", help="Exchange name")
@click.option("--market", default="usdm", help="Market type")
@click.option("--price-type", "price_type", default="trade", help="Price type")
@click.option(
    "--wait", is_flag=True, help="Wait for backfill to complete synchronously"
)
...
def data_backfill(
    ...
    console.print(
        f"[bold]Data backfill[/bold] {symbol} {timeframe} exchange={exchange}"
    )
    ...
    if start_ms is None or end_ms is None:
        raise RuntimeError("backfill window parser returned incomplete bounds")
    ...
    console.print(f"  from={from_date} to={to_date or 'today'}")
    console.print("[dim]Use --wait to fetch synchronously, or check:[/dim]")
...
@click.option(
    ...
    default=None,
    type=int,
    help="Max parallel workers (default: half CPU cores)",
)
@click.option(
    "--chunked", is_flag=True, help="Use chunked parallel fetch for large ranges"
)
...
def data_inspect(
    ...
    if not bars:
        console.print(
            f"[dim](no bars in range {from_date} → {to_date or 'today'})[/dim]"
        )
        return
    ...
    tbl = Table(
        title=f"Candles: {symbol} {timeframe} ({from_date} → {to_date or 'today'})"
    )
...
def data_doctor(
    ...
    if classification == "DUPLICATE_TIMESTAMPS":
        console.print(
            "[red]  Critical: duplicate timestamps found in canonical coverage.[/red]"
        )
