...
def schema(path: str | None) -> None:
    ...
    for table in tables:
        ...
        for col in col_cursor.fetchall():
            console.print(
                f"    {col[1]} {col[2]}  nullable={not col[3]} default={col[4]}"
            )
        table_storage.close()
...
@storage.command("health")
@click.option("--path", type=click.Path(), default=None)
def health(path: str | None) -> None:
    """Check storage migrations, required indexes and event schema compatibility."""

    from openpine.config import OpenPineConfig
    from openpine.storage import MigrationRunner, SQLiteStorage
    from openpine.storage.db_health import schema_health

    db_path = OpenPineConfig.load().sqlite_path if path is None else Path(path)
    console.print(f"[bold]Storage health[/bold] - path={db_path}")
    with SQLiteStorage(db_path) as storage_db:
        MigrationRunner().run_migrations(storage_db)
        report = schema_health(storage_db)

    console.print(f"schema_contract: {report.schema_contract or 'unknown'}")
    console.print(f"tables: {report.table_count}")
    console.print(f"indexes: {report.index_count}")
    console.print(f"applied migrations: {', '.join(report.applied_versions) or 'none'}")
    console.print(f"pending migrations: {', '.join(report.pending_versions) or 'none'}")
    if report.missing_indexes:
        console.print(f"[red]required indexes missing:[/red] {', '.join(report.missing_indexes)}")
    else:
        console.print("required indexes: ok")
    console.print(
        "events schema: compatible"
        if report.event_schema_compatible
        else "[red]events schema: incompatible[/red]"
    )
    if not report.ok:
        raise SystemExit(1)
