...
async def create_source(
    ...
    try:
        ...
        if existing:
            raise HTTPException(
                409, f"Pine source with name '{body.name}' already exists"
            )
    except KeyError:
        pass
...
async def delete_source(
    ...
    try:
        state.storage.execute(
            "DELETE FROM compile_artifacts WHERE source_id = ?", (source_id,)
        )
        state.storage.execute(
            "DELETE FROM pine_artifacts WHERE pine_id = ?", (source_id,)
        )
        state.storage.commit()
    except Exception as exc:
        rollback = getattr(state.storage, "rollback", None)
        if callable(rollback):
            rollback()
        log.warning(
            "pine_source_artifact_rows_cleanup_failed",
            source_id=source_id,
            error=str(exc),
        )
        raise HTTPException(500, f"Failed to clean Pine source artifact rows: {exc}") from exc
    try:
        import shutil
        ...
        log.warning(
            "pine_source_artifact_dir_cleanup_failed",
            source_id=source_id,
            error=str(exc),
        )
        raise HTTPException(500, f"Failed to clean Pine source artifact files: {exc}") from exc
...
async def delete_source_preview(
    ...
    return {
        ...
        "resources": {
            "source_rows": 1,
            "compile_artifacts": count(
                "SELECT COUNT(*) FROM compile_artifacts WHERE source_id = ?", (source.id,)
            ),
            "pine_artifacts": count(
                "SELECT COUNT(*) FROM pine_artifacts WHERE pine_id = ?", (source.id,)
            ),
            "artifact_files": artifact_files,
            "strategies_referencing": count(
                "SELECT COUNT(*) FROM strategy_instances WHERE pine_id = ?",
                (source.id,),
            ),
        },
    }
