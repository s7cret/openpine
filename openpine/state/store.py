...
class SnapshotMetadata:
    ...
    reason: str = (
        "scheduled"  # scheduled / position_change / order_event / signal / manual / rebuild
    )
...
class StateStore:
    ...
    def _validate_path_component(component: str) -> None:
        ...
        if (
            ...
            raise ValueError(f"Path component escapes state storage root: {component}")
    ...
    def _path_under_root(self, *parts: str) -> Path:
        ...
        try:
            ...
            raise ValueError(f"Path escapes state storage root: {path}") from exc
        return path

    def _state_dir(self, strategy_id: str) -> Path:
        return self._path_under_root(f"strategy_id={strategy_id}")

    def _snapshot_path(self, strategy_id: str, snapshot_id: str) -> Path:
        return self._path_under_root(
            f"strategy_id={strategy_id}", f"snap_{snapshot_id}.state.msgpack.zst"
        )

    def _debug_path(self, strategy_id: str, snapshot_id: str) -> Path:
        return self._path_under_root(
            f"strategy_id={strategy_id}", f"snap_{snapshot_id}.debug.json"
        )
    ...
    @staticmethod
    def _pickle_state_enabled() -> bool:
        return os.environ.get("OPENPINE_ALLOW_PICKLE_STATE") == "1"

    @staticmethod
    def _pickle_state_error() -> InvalidSnapshotError:
    ...
    def load_latest_compatible(
        self, strategy_id: str, **filters: Any
    ) -> StrategyState | None:
    ...
    def _persist_metadata_index(self) -> None:
        payload = {
            ...
            "snapshots": [
                meta.to_dict() for metas in self._snapshots.values() for meta in metas
            ],
        }
