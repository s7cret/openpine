...
class LocalOptimizerAdapter:
    ...
    def _run_local_optimizer(
        ...
        if module is None:
            return self._failed_result(
                optimization_id, config, "optimizer module not loaded"
            )
    ...
    def _normalize_optimizer_result(
        ...
        if getattr(recommended, "status", None) != "completed":
            recommended = None
        best_params = (
            dict(getattr(recommended, "params", {}) or {}) if recommended else {}
        )
    ...
    def _trial_status_counts(
        self, raw_result: Any, trials: tuple[Any, ...]
    ) -> dict[str, int]:
