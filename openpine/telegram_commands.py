...
TELEGRAM_COMMANDS: tuple[TelegramCommandSpec, ...] = (
    ...
    _cmd(
        ...
        "/storage_init [--path PATH] [--dry-run]",
        "Initialize storage.",
    ),
    _cmd(
        "/storage_migrate",
    ...
    _cmd(
        ...
        "Storage backup",
        ("storage", "backup"),
        "/storage_backup --out FILE.tar.gz",
        "Create backup.",
    ),
    _cmd(
        "/storage_restore",
        ...
        "Storage restore",
        ("storage", "restore"),
        "/storage_restore FILE.tar.gz [--target DIR]",
        "Restore backup.",
    ),
    _cmd(
        "/pines",
    ...
    _cmd(
        ...
        "Pine activate",
        ("pine", "activate"),
        "/pine_activate NAME ARTIFACT_ID",
        "Activate artifact.",
    ),
    _cmd(
        "/pine_rollback",
        ...
        "Pine rollback",
        ("pine", "rollback"),
        "/pine_rollback NAME [--to-version ARTIFACT_ID]",
        "Rollback artifact.",
    ),
    _cmd(
        "/pine_remove",
    ...
    _cmd(
        ...
        "Strategy create",
        ("strategy", "create"),
        "/strategy_create [ID] --pine NAME --symbol BTCUSDT --timeframe 1m [--mode paper] [--param k=v]",
        "Create strategy.",
    ),
    _cmd(
        "/strategy_update",
        ...
        "Strategy update",
        ("strategy", "update"),
        "/strategy_update STRATEGY_ID --param k=v",
    ...
    _cmd(
        ...
        "Strategy backtest",
        ("strategy", "backtest"),
        "/strategy_backtest STRATEGY_ID [--from DATE] [--to DATE]",
        "Run backtest.",
    ),
    _cmd(
        "/strategy_replay",
        ...
        "Strategy replay",
        ("strategy", "replay"),
        "/strategy_replay STRATEGY_ID [--from DATE] [--to DATE]",
        "Run replay.",
    ),
    _cmd(
        "/strategy_paper",
        ...
        "Strategy paper",
        ("strategy", "paper"),
        "/strategy_paper STRATEGY_ID start|stop",
        "Control paper trading.",
    ),
    _cmd(
        "/strategy_live",
        "strategy",
        "Strategy live",
        ("strategy", "live"),
        "/strategy_live STRATEGY_ID enable|start|stop",
        "Control live trading.",
    ),
    _cmd(
        "/strategy_error_clear",
        "strategy",
        "Strategy error clear",
        ("strategy", "error"),
        "/strategy_error_clear STRATEGY_ID clear [--to paused|disabled]",
        "Clear error state.",
    ),
    _cmd(
        "/data_status",
        ...
        "Data status",
        ("data", "status"),
        "/data_status [SYMBOL] [--exchange binance] [--timeframe 1m]",
    ...
    _cmd(
        ...
        "Data gaps",
        ("data", "gaps"),
        "/data_gaps SYMBOL TIMEFRAME [--exchange binance] [--market usdm]",
        "Find data gaps.",
    ),
    _cmd(
        "/data_repair",
        ...
        "Data repair",
        ("data", "repair"),
        "/data_repair SYMBOL TIMEFRAME --from MS --to MS [--exchange binance]",
        "Queue repair/backfill.",
    ),
    _cmd(
        "/data_backfill",
        "data",
        "Data backfill",
        ("data", "backfill"),
        "/data_backfill SYMBOL TIMEFRAME --from YYYY-MM-DD [--to YYYY-MM-DD]",
        "Backfill candles.",
    ),
    _cmd(
        "/data_inspect",
        ...
        "Data inspect",
        ("data", "inspect"),
        "/data_inspect SYMBOL TIMEFRAME --from YYYY-MM-DD [--to YYYY-MM-DD]",
        "Inspect candle files.",
    ),
    _cmd(
        "/data_providers",
        "data",
    ...
    _cmd(
        ...
        "Account add",
        ("accounts", "add"),
        "/account_add --name N --exchange binance --api-key KEY --secret SECRET [--mode paper]",
        "Add account.",
    ),
    _cmd(
        "/account_test",
    ...
    _cmd(
        "/job_enqueue_live_bar",
        "jobs",
        "Enqueue live bar",
        ("jobs", "enqueue-live-bar"),
        "/job_enqueue_live_bar --strategy ID --bar-time MS [--dry-run]",
        "Enqueue live-bar job.",
    ),
    _cmd(
        "/queue",
        "jobs",
    ...
    _cmd(
        ...
        "State rebuild",
        ("state", "rebuild"),
        "/state_rebuild STRATEGY_ID [--from-bar MS]",
        "Rebuild state.",
    ),
    _cmd(
        "/risk",
    ...
    _cmd(
        ...
        "Report export",
        ("reports", "export"),
        "/report_export REPORT_ID [--format json|csv]",
        "Export report.",
    ),
    _cmd(
        "/plugins",
    ...
    _cmd(
        ...
        "Plugin enable",
        ("plugins", "enable"),
        "/plugin_enable telegram [--chat-id ID]",
        "Enable plugin.",
    ),
    _cmd(
        "/plugin_test",
        ...
        "Plugin test",
        ("plugins", "test"),
        "/plugin_test telegram --chat-id ID",
        "Test plugin.",
    ),
    _cmd(
        "/universe",
    ...
    _cmd(
        ...
        "Optimizer dry run",
        ("optimizer", "dry-run"),
        "/optimizer_dry_run --strategy ID --trials N",
        "Validate optimizer route.",
    ),
    _cmd(
        "/config",
        "config",
    ...
    _cmd(
        "/events_schema_validate",
        "events",
        "Event schema validate",
        ("events", "schema", "validate"),
        "/events_schema_validate EVENT_TYPE",
...
def home_menu_keyboard() -> dict[str, list[list[dict[str, str]]]]:
    ...
    return inline_keyboard(
        (
            (
                _button("Status", "op:status"),
                _button("Strategies", "op:strategies:list"),
            ),
            (
                _button("📁 Pine Sources", "op:pine:list"),
                _button("📊 Strategies", "op:strategies:list"),
            ),
            (
                _button("Risk", "op:risk:status"),
                _button("Data/Jobs", "op:menu:data_jobs"),
            ),
            (
                _button("Reports", "op:reports:list"),
                _button("Plugins", "op:plugins:list"),
            ),
            (
                _button("Streams", "op:streams:status"),
                _button("Config", "op:config:show"),
            ),
        )
    )
...
def strategy_list_keyboard(
    ...
    for strat in strategies:
        ...
        pause_cb = (
            f"op:strat:resume:{sid}" if status == "paused" else f"op:strat:pause:{sid}"
        )
        live_label = "Live stop" if status == "live" else "Live enable"
        live_cb = (
            f"op:strat:live_stop:{sid}"
            if status == "live"
            else f"op:strat:live_enable:{sid}"
        )
        paper_label = "Paper stop" if status == "paper" else "Paper start"
        paper_cb = (
            f"op:strat:paper_stop:{sid}"
            if status == "paper"
            else f"op:strat:paper_start:{sid}"
        )
        ...
        rows.append((_button("🗑 Delete", f"op:strat:confirm_delete:{sid}"),))
...
def pine_list_keyboard(
    ...
    for source in sources:
        ...
        rows.append((_button(f"{active_icon} {name}", f"op:pine:show:{sid}"),))
...
def data_jobs_keyboard() -> dict[str, list[list[dict[str, str]]]]:
    ...
    return inline_keyboard(
        (
            (
                _button("Data status", "op:data:status"),
                _button("Data plan", "op:data:plan"),
            ),
            (_button("Jobs", "op:jobs:list"), _button("Queue", "op:queue:status")),
            (
                _button("Workers", "op:workers:status"),
                _button("Providers", "op:providers:list"),
            ),
        )
    )
...
def _clean_callback_arg(value: str) -> str:
    ...
    if not value or ":" in value:
        raise TelegramCommandError(
            "callback argument must be non-empty and cannot contain ':'"
        )
    return value
