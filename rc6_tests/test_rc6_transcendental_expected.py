"""Manual angle, domain and instrument rounding tables through five paths."""
...
def test_independent_transcendental_expected(case, path, observations):
    ...
    assert first_difference(
        expected, {"compile": observation["compile"], "events": observation["events"]},
        case["tolerance"],
    ) is None
...
def observations():
    ...
    write_json(Path(output) / "builtin-transcendental-surface.json", surface)
    for path, results in observed.items():
        ...
        report = builtin_evidence_report(
            surface, CORPUS, results, corpus_hash=LOCK["content_hash"], assignments=assignments
        )
        directory = Path(output) / "builtin-transcendental-reports" / path
...
def test_manual_transcendental_table_keeps_versions_domains_and_inferred_na_explicit():
    cases = MANIFEST["cases"]
    assert {case["pine_version"] for case in cases} == {5, 6}
    assert sum("outside-domain" in case["id"] for case in cases) == 4
    assert sum("INFERRED_FROM_GENERAL_DOCS" in case["oracle"]["provenance"] for case in cases) == 18
