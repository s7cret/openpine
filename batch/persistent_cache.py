...
def default_cache_dir(root: Path) -> Path:
    return Path(os.environ.get("OPENPINE_BATCH_CACHE_DIR", root / ".openpine_batch_cache"))
...
def save_bars(cache_dir: Path, key_payload: dict[str, Any], bars: list[Any]) -> dict[str, Any]:
    key = cache_key(key_payload)
    meta_path, data_path = _paths(cache_dir, "bars", key)
    rows = [
        {
...
def save_tv_corpus(
    ...
    _write_json_atomic(cache_meta, meta_path)
    return cache_meta
